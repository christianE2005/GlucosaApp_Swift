...
struct InsightsView: View {
        ...
        NavigationView {
            ScrollView {
                LazyVStack(spacing: 20) {
                    // 🚨 Banner de nuevos datos IA
                    if showNewAIDataBanner {
                        NewAIDataBanner(count: newAIDataCount) {
                            withAnimation {
                                showNewAIDataBanner = false
                            }
                        }
                    }
            ...
            .onReceive(NotificationCenter.default.publisher(for: .newAIDataAdded)) { _ in
                checkForNewAIData()
            }
        }
    }
    ...
    // MARK: - Función para detectar nuevos datos de IA
    private func checkForNewAIData() {
        let currentAICount = aiAnalyzedMeals.count
        if currentAICount > newAIDataCount {
            newAIDataCount = currentAICount
            withAnimation(.spring()) {
                showNewAIDataBanner = true
            }
...
struct EnhancedTrendsAnalysisCard: View {
    ...
    private func DynamicChartSection() -> some View {
        VStack(spacing: 16) {
            ...
            // 🧠 Leyenda para datos IA
            if meals.contains(where: { $0.isAIAnalyzed }) {
                AIDataLegend()
            }
        }
    }
...
struct ChartTypeButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .padding(.vertical, 6)
            .background(isSelected ? chartType.color : Color(UIColor.systemGray6))
...
// MARK: - Chart Data Point with AI indicator
struct ChartDataPoint {
    let date: Date
    let value: Double
    let formattedDate: String
    let color: Color
    let isAIAnalyzed: Bool // 🧠 Nuevo: Indicador de análisis IA
    
    init(date: Date, value: Double, formattedDate: String, color: Color, isAIAnalyzed: Bool = false) {
        self.date = date
        self.value = value
    ...
    var body: some View {
        let caloriesData = meals.compactMap { meal -> ChartDataPoint? in
            ...
            return ChartDataPoint(
                ...
                isAIAnalyzed: meal.isAIAnalyzed // 🧠 Incluir indicador IA
            )
        }
        .sorted { $0.date < $1.date }
        .suffix(10)
        ...
        if caloriesData.isEmpty {
            ...
            GeometryReader { geometry in
                ...
                ZStack {
                    ...
                    // Puntos de datos con indicador IA
                    ForEach(caloriesData.indices, id: \.self) { index in
                        ...
                        ZStack {
                            ...
                            // 🧠 Indicador especial para datos IA
                            if dataPoint.isAIAnalyzed {
                                Circle()
...
struct ChartDetailsView: View {
    ...
    var body: some View {
        NavigationView {
            ScrollView {
                VStack(spacing: 20) {
                    Text("Detalles de \(chartType.rawValue)")
                        .font(.title2)
                        .fontWeight(.bold)
                        .padding()
                    ...
                    // Additional details
                    Text("Análisis detallado próximamente")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .padding()
                }
            }
            .navigationTitle("Detalles del Gráfico")
            .navigationBarTitleDisplayMode(.inline)
...
// MARK: - Banner de nuevos datos IA
struct NewAIDataBanner: View {
    let count: Int
    let onDismiss: () -> Void
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("🧠 ¡Datos IA Actualizados!")
                    .font(.headline)
                    .fontWeight(.semibold)
                    .foregroundColor(.purple)
                
                Text("Ahora tienes \(count) comidas analizadas con IA en tus gráficas")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
...
// MARK: - Leyenda para datos IA
struct AIDataLegend: View {
    var body: some View {
        HStack(spacing: 16) {
            HStack(spacing: 6) {
                ZStack {
